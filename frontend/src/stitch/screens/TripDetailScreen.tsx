  ...
  const generateSmartSummary = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          systemInstruction: "You are a specialized dispatcher assistant for NEMT drivers. Provide only clear, short bullet points. Do not include introductory text.",
        }
      });
    ...
    <div className="fixed inset-0 z-[55] bg-gray-50 flex flex-col max-w-md mx-auto h-screen">
      ...
      <div className="bg-white px-6 py-6 flex items-center justify-between border-b border-gray-100">
        ...
        <div className="text-center">
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Assignment Details</p>
          <div className="flex items-center gap-2">
            <span className="text-sm font-black text-gray-900">{trip.id}</span>
            <button className="text-teal-500 active:scale-90 transition-transform"><Copy size={12}/></button>
          </div>
        </div>
        <button className="p-2 -mr-2 text-gray-400 active:bg-gray-50 rounded-full transition-colors"><MoreVertical size={24} /></button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-6 pb-40">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-teal-500 animate-pulse shadow-[0_0_8px_rgba(14,165,233,0.5)]" />
            <span className="text-[11px] font-black text-gray-700 uppercase tracking-widest">{trip.status}</span>
          </div>
          <div className="flex gap-2">
             <div className="bg-teal-50 text-teal-600 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-tight border border-teal-100">
               {trip.type} RIDE
             </div>
             {trip.client.mobilityNeeds.includes('Wheelchair') && (
               <div className="bg-purple-50 text-purple-600 px-3 py-1.5 rounded-full text-[10px] font-black uppercase flex items-center gap-1.5 border border-purple-100 shadow-sm">
        ...
        <div className="bg-white rounded-[40px] border border-gray-100 shadow-sm overflow-hidden group">
          <div className="p-8 space-y-6">
            <div className="flex items-center gap-6">
              <div className="w-24 h-24 bg-teal-50 rounded-[34px] flex items-center justify-center text-teal-500 border-4 border-white shadow-xl relative overflow-hidden group-hover:scale-105 transition-transform duration-500">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button className="flex items-center justify-center gap-2.5 py-4 bg-teal-500 text-white rounded-[24px] font-black text-[11px] uppercase tracking-widest active:scale-95 transition-all shadow-xl shadow-teal-100">
                <Phone size={18} /> Call Member
              </button>
              <button 
                onClick={fetchCareInsights}
                className="flex items-center justify-center gap-2.5 py-4 bg-white border-2 border-teal-100 rounded-[24px] text-teal-600 font-black text-[11px] uppercase tracking-widest active:scale-95 transition-all shadow-sm"
              >
          ...
          <div className="bg-amber-50/50 p-8 border-t border-amber-100">
            <div className="flex items-center justify-between mb-5">
              ...
              <div className="flex gap-2.5">
                <button 
                  ...
                  className="flex items-center gap-2 bg-teal-500 px-4 py-2 rounded-full text-[10px] font-black text-white uppercase tracking-widest active:scale-95 transition-all shadow-xl shadow-teal-200"
                >
            ...
            {smartSummary ? (
              <div className="space-y-3 animate-in fade-in slide-in-from-top-3">
                ...
                <button onClick={() => setSmartSummary(null)} className="text-[10px] font-black text-teal-600 uppercase tracking-widest mt-3 ml-2 flex items-center gap-1.5">
            ...
            <div className="flex gap-6 relative">
              ...
              <div className="space-y-10 flex-1">
                <div>
                  ...
                  <p className="text-[11px] text-teal-600 font-black uppercase mt-2 tracking-widest flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-2 gap-4 pb-12">
           <button 
             ...
             className="flex items-center justify-center gap-3 py-5 bg-white border border-gray-100 rounded-[28px] text-teal-600 font-black text-[11px] uppercase tracking-widest active:scale-95 shadow-sm transition-all"
           >
              <div className="p-2 bg-teal-50 rounded-xl"><MessageSquare size={18} /></div> Dispatch
           </button>
      ...
      {/* Care Insights Modal Overlay (Req 6.2) */}
      {showCareInsights && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
           <div className="absolute inset-0 bg-gray-900/80 backdrop-blur-xl" onClick={() => setShowCareInsights(false)} />
           <div className="bg-white w-full rounded-[44px] shadow-2xl relative p-10 animate-in zoom-in-95 duration-300">
              <div className="flex items-center justify-between mb-8">
                 <div className="flex items-center gap-3">
                    <div className="w-12 h-12 bg-teal-500 text-white rounded-2xl flex items-center justify-center shadow-lg"><Sparkles size={24}/></div>
                    <h3 className="text-2xl font-black text-gray-900">Care Insights</h3>
                 </div>
                 <button onClick={() => setShowCareInsights(false)} className="p-3 bg-gray-50 rounded-2xl text-gray-400"><XCircle size={20}/></button>
              </div>
              ...
              {isLoadingCare ? (
                <div className="py-20 flex flex-col items-center gap-6">
                   ...
                   <p className="text-xs font-black text-teal-600 uppercase tracking-[0.2em] animate-pulse">Analyzing Member History...</p>
                </div>
              ) : careInsights && (
                <div className="space-y-6">
                   <div className="grid grid-cols-2 gap-4">
                      <div className="p-6 bg-teal-50 rounded-3xl border border-teal-100">
                         <p className="text-[9px] font-black text-teal-600 uppercase tracking-widest mb-2">Atmosphere</p>
                         <p className="text-sm font-black text-teal-900">{careInsights.temp}</p>
                      </div>
                   ...
                   <button 
                     ...
                     className="w-full bg-teal-500 text-white font-black py-5 rounded-[28px] shadow-xl shadow-teal-100 mt-4 active:scale-95 transition-all"
                   >
      ...
      <div className="p-8 bg-white border-t border-gray-100 fixed bottom-0 left-0 right-0 max-w-md mx-auto shadow-[0_-20px_60px_rgba(0,0,0,0.08)]">
         <button 
           ...
           className="w-full bg-teal-500 text-white font-black py-5 rounded-[32px] shadow-2xl shadow-teal-200 text-xl active:scale-95 transition-all flex items-center justify-center gap-4 group"
         >
