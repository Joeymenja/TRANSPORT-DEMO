...
const PerformanceScreen: React.FC<PerformanceScreenProps> = ({ onBack }) => {
  ...
  const [forecast, setForecast] = useState<{ earnings: string; tip: string } | null>(null);
  ...
  const generateInsight = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: "Analyze NEMT driver stats: 98.4% compliance. Provide 1 coaching insight and a weekly earnings forecast. Format as JSON: {insight: string, forecast: {earnings: string, tip: string}}",
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-8 space-y-8 pb-32">
        ...
        <section className="space-y-4">
           ...
           <div className="bg-white p-8 rounded-[48px] border border-gray-100 shadow-sm flex justify-between relative overflow-hidden">
              ...
              <div className="absolute top-1/2 left-8 w-[60%] h-1 bg-teal-500 -translate-y-1/2 rounded-full shadow-[0_0_10px_#0ea5e9]" />
              {milestones.map((m) => (
                <div key={m.id} className="relative z-10 flex flex-col items-center gap-3">
                   <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border-4 border-white shadow-xl transition-all ${m.status === 'completed' ? 'bg-teal-500 text-white' : m.status === 'current' ? 'bg-white text-teal-500 scale-110' : 'bg-gray-100 text-gray-300'}`}>
                      <m.icon size={24} className={m.status === 'current' ? 'animate-pulse' : ''} />
                   </div>
                   <span className={`text-[8px] font-black uppercase tracking-tighter text-center max-w-[50px] leading-tight ${m.status === 'locked' ? 'text-gray-300' : 'text-gray-900'}`}>{m.label}</span>
                </div>
              ))}
           </div>
        </section>
             ...
             <div className="relative z-10 space-y-6">
                ...
                <div className="p-5 bg-white/10 rounded-[32px] border border-white/10 backdrop-blur-md">
                   <div className="flex items-center gap-3 mb-2">
                      ...
                      <p className="text-[10px] font-black text-sky-300 uppercase tracking-widest">Growth Recommendation</p>
                   </div>
        ...
        <div className="p-8 bg-white rounded-[44px] border border-gray-100 shadow-sm space-y-6">
           ...
           <div className="h-3 w-full bg-gray-50 rounded-full overflow-hidden">
              <div className="h-full w-[84%] bg-teal-500 rounded-full shadow-[0_0_10px_#0ea5e9]" />
           </div>
        </div>
      </div>
    </div>
