    ...
    <div className="h-full w-full bg-white/95 backdrop-blur-xl flex flex-col animate-in slide-in-from-right duration-300">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-6 pb-24">
        <div className="flex items-center justify-between px-2">
          ...
          <button onClick={onMarkRead} className="text-[10px] font-black text-teal-500 uppercase tracking-widest">Clear Badge</button>
        </div>
        ...
        <div className="space-y-3">
          {items.length === 0 ? (
            ...
            <div 
              ...
              className={`p-5 rounded-[32px] border transition-all active:scale-[0.98] relative overflow-hidden group ${item.read ? 'bg-white border-gray-50 opacity-70' : 'bg-white border-teal-100 shadow-lg shadow-teal-100/50'}`}
            >
              {!item.read && <div className="absolute top-0 left-0 w-1.5 h-full bg-teal-500" />}
              
              <div className="flex items-start gap-4">
                <div className={`p-3 rounded-2xl ${item.type === 'trip' ? 'bg-teal-50 text-teal-500' : 'bg-amber-50 text-amber-500'}`}>
                ...
                <ChevronRight size={16} className="text-gray-300 mt-1 group-hover:text-teal-500 transition-colors" />
              </div>
            </div>
          ))}
        </div>
