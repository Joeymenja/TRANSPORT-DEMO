    ...
    <div className="fixed inset-0 z-[65] bg-white flex flex-col max-w-md mx-auto h-screen">
      ...
      <div className="bg-white px-6 py-5 flex items-center justify-between border-b border-gray-50 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          <button className="p-2 text-sky-500 bg-sky-50 rounded-xl active:scale-90 transition-all"><Phone size={20}/></button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4 no-scrollbar bg-gray-50">
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] p-4 rounded-[24px] ${msg.sender === 'me' ? 'bg-sky-500 text-white rounded-br-none shadow-lg shadow-sky-100' : 'bg-white text-gray-800 rounded-bl-none shadow-sm border border-gray-100'}`}>
              <p className="text-sm font-medium leading-relaxed">{msg.text}</p>
              <p className={`text-[9px] font-bold mt-2 uppercase tracking-widest ${msg.sender === 'me' ? 'text-sky-100' : 'text-gray-400'}`}>
                {msg.time}
              </p>
            </div>
          </div>
        ...
        {isTyping && (
          <div className="flex justify-start">
             <div className="bg-white p-3 rounded-[20px] rounded-bl-none shadow-sm border border-gray-100 flex items-center gap-1">
                ...
                <div className="w-1.5 h-1.5 bg-sky-400 rounded-full animate-bounce" style={{ animationDelay: '200ms' }} />
                <div className="w-1.5 h-1.5 bg-sky-400 rounded-full animate-bounce" style={{ animationDelay: '400ms' }} />
             </div>
          </div>
        )}
      ...
      <div className="p-6 bg-white border-t border-gray-50">
        <div className="flex items-end gap-3 bg-gray-50 p-2 rounded-[28px] border border-gray-100">
          <button className="p-3 text-gray-400 hover:text-sky-500 transition-colors"><Paperclip size={20}/></button>
          <textarea 
            rows={1}
            value={message}
          ...
          />
          {message.trim() && (
            <button onClick={handleSend} className="p-3 bg-sky-500 text-white rounded-2xl shadow-lg active:scale-90 transition-all"><Send size={20} /></button>
          )}
        </div>
      </div>
    </div>
