    ...
    <div className="pb-32">
      <div className="p-8 bg-white border-b border-gray-100">
        <div className="flex items-center gap-3 mb-2">
           <div className="p-2 bg-sky-50 text-sky-500 rounded-lg"><Clock size={20}/></div>
      ...
      <div className="p-6 space-y-8">
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between px-2">
            ...
            <button onClick={copyToAll} className="flex items-center gap-1.5 text-[10px] font-black text-sky-500 uppercase tracking-widest active:scale-95 transition-all">
          ...
          <div className="space-y-3">
            {schedule.map((item, idx) => (
              <div key={idx} className={`bg-white p-5 rounded-[28px] border transition-all flex items-center justify-between ${item.enabled ? 'border-sky-100 shadow-sm' : 'border-gray-50 opacity-60'}`}>
                <div className="flex items-center gap-4">
                  <button 
                    ...
                    className={`w-12 h-12 rounded-[18px] flex items-center justify-center transition-all ${item.enabled ? 'bg-sky-500 text-white shadow-lg shadow-sky-100' : 'bg-gray-100 text-gray-400'}`}
                  >
                  ...
                  <div>
                    ...
                    {item.enabled ? (
                      <div className="flex items-center gap-1 mt-0.5">
                        <span className="text-[10px] font-black text-sky-600 uppercase tracking-tighter">{item.start}</span>
                        <span className="text-[9px] font-bold text-gray-300">to</span>
                        <span className="text-[10px] font-black text-sky-600 uppercase tracking-tighter">{item.end}</span>
                      </div>
                    ) : (
                ...
                {item.enabled && (
                  <div className="flex items-center gap-2">
                    <button className="p-2.5 bg-gray-50 rounded-xl text-gray-400 hover:text-sky-500 transition-colors">
                      <Clock size={16} />
                    </button>
                  </div>
                )}
        ...
        <section className="space-y-4 pt-4">
          <div className="flex items-center justify-between px-2">
            ...
            <button 
              ...
              className="flex items-center gap-2 bg-sky-50 text-sky-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all"
            >
      ...
      <div className="p-6 fixed bottom-20 left-0 right-0 max-w-md mx-auto">
        <button className="w-full bg-sky-500 text-white font-black py-5 rounded-[32px] shadow-2xl shadow-sky-200 active:scale-95 transition-all">
