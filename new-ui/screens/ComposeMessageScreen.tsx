    ...
    <div className="fixed inset-0 z-[70] bg-white flex flex-col max-w-md mx-auto h-screen">
      <div className="bg-white px-6 py-6 border-b border-gray-50 flex items-center justify-between">
        ...
        {isSent ? (
          ...
          <button 
            ...
            className={`p-2 rounded-xl transition-all ${(!recipient || !message) ? 'text-gray-300' : 'bg-sky-50 text-sky-500 shadow-sm active:scale-90'}`}
          >
            <Send size={24}/>
          </button>
        )}
      ...
      <div className="flex-1 flex flex-col p-6 space-y-6">
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
            {recipients.map((r) => (
              <button 
                ...
                className={`flex-shrink-0 px-4 py-2 rounded-full border transition-all flex items-center gap-2 ${recipient === r.name ? 'bg-sky-500 border-sky-500 text-white shadow-lg shadow-sky-100' : 'bg-white border-gray-100 text-gray-500'}`}
              >
                <User size={14} />
        ...
        <textarea 
          ...
          className="flex-1 w-full p-4 bg-gray-50 rounded-[32px] border border-gray-100 font-medium text-gray-800 text-sm focus:bg-white transition-all resize-none no-scrollbar focus:ring-2 focus:ring-sky-500/20"
        />

        <div className="flex items-center justify-between gap-2 p-2 bg-gray-50 rounded-[24px] border border-gray-100">
           <div className="flex items-center gap-1">
              <button className="p-3 text-gray-400 hover:text-sky-500"><Paperclip size={20}/></button>
              <button className="p-3 text-gray-400 hover:text-sky-500"><Camera size={20}/></button>
              <button className="p-3 text-gray-400 hover:text-sky-500"><MapPin size={20}/></button>
           </div>
      ...
      {isSent && (
        <div className="fixed inset-0 bg-white/60 backdrop-blur-md z-[80] flex items-center justify-center">
           <div className="bg-sky-500 text-white px-8 py-4 rounded-[24px] shadow-2xl flex items-center gap-4">
