      ...
      <div className="fixed inset-0 z-[130] bg-black flex flex-col">
        <div className="p-8 flex items-center justify-between text-white relative z-10">
          ...
          <div className="text-center">
             <p className="text-[10px] font-black uppercase tracking-[0.2em] text-sky-400 mb-1">AI Safety HUD</p>
        ...
        <div className="flex-1 flex items-center justify-center p-8 relative">
           <div className="w-full aspect-square border-2 border-white/20 rounded-[60px] relative overflow-hidden flex items-center justify-center">
              ...
              <div className="absolute top-0 left-0 right-0 h-1 bg-sky-400 shadow-[0_0_20px_rgba(56,189,248,0.8)] animate-[scan_2.5s_ease-in-out_infinite]" />
              <style>{`
                @keyframes scan {
                  0% { top: 0; }
                  50% { top: 100%; }
        ...
        <div className="p-12 pb-20 flex flex-col items-center gap-6">
           ...
           >
              {isAnalyzing ? <Loader2 className="animate-spin text-sky-500" size={32} /> : <div className="w-12 h-12 bg-sky-500 rounded-full" />}
           </button>
        </div>
      </div>
    );
    ...
    <div className="fixed inset-0 z-[120] bg-white flex flex-col max-w-md mx-auto h-screen">
      ...
      <div className="flex-1 flex flex-col p-8 space-y-8 overflow-y-auto no-scrollbar">
        {!guideData && (
          <div className="flex-1 flex flex-col items-center justify-center text-center">
            <div className="w-24 h-24 bg-sky-50 rounded-[32px] flex items-center justify-center text-sky-400 mb-6 border-2 border-dashed border-sky-100">
          ...
          <div className="space-y-6">
            <div className="p-6 bg-sky-50 rounded-[32px] border border-sky-100">
               <div className="flex items-center gap-3 mb-4">
                  <div className="p-2 bg-white rounded-lg shadow-sm"><Zap size={16} className="text-sky-500" /></div>
                  <h4 className="text-[10px] font-black text-sky-600 uppercase tracking-widest">Device Identified</h4>
               </div>
               <p className="text-xl font-black text-sky-900">{guideData.model}</p>
            </div>
            ...
            <div className="space-y-3">
              {guideData.steps.map((step: string, i: number) => (
                <div key={i} className="flex items-start gap-4 p-5 bg-white border border-gray-100 rounded-[24px] shadow-sm">
                  <div className="w-6 h-6 bg-sky-500 text-white rounded-full flex items-center justify-center font-black text-xs shrink-0 mt-0.5">{i + 1}</div>
      ...
      <div className="p-8 border-t border-gray-100">
        <button 
          ...
          className="w-full bg-sky-500 text-white font-black py-5 rounded-[32px] shadow-2xl shadow-sky-200 text-lg flex items-center justify-center gap-3 active:scale-95 transition-all"
        >
          <Camera size={24} />
