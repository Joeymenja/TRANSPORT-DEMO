...
const Layout: React.FC<LayoutProps> = ({ activeTab, setActiveTab, children, title, rightAction }) => {
  ...
  useEffect(() => {
    ...
    // Subscribe to queue for badge count
          ...
          <nav className="p-4 space-y-1">
             {menuItems.map((item, idx) => (
               <button 
                 ...
                 className="w-full flex items-center gap-4 p-4 rounded-2xl hover:bg-sky-50 text-gray-700 active:scale-95 transition-all group"
               >
                 <div className="p-2 bg-gray-50 rounded-xl group-hover:bg-sky-100 group-hover:text-sky-600 transition-colors">
                    <item.icon size={20} />
                 </div>
      ...
      <header className="bg-white border-b border-gray-200 px-4 py-4 flex items-center justify-between z-10 shadow-sm">
        <div className="flex items-center gap-3">
          <button 
            ...
            className="p-2 -ml-2 text-gray-400 hover:text-sky-500 active:scale-90 transition-all"
          >
            <Menu size={24} />
          </button>
            ...
            >
              {isOnline ? (
                <div className="flex items-center gap-1">
                  <div className={`w-2 h-2 rounded-full ${queueCount > 0 ? 'bg-sky-500 animate-pulse' : 'bg-green-500'}`} />
                  <span className={`text-[8px] font-black uppercase tracking-widest ${queueCount > 0 ? 'text-sky-600' : 'text-green-600'}`}>
                    {queueCount > 0 ? `${queueCount} Pending` : 'Sync Active'}
                  </span>
                </div>
              ) : (
                <div className="flex items-center gap-1">
      ...
      <nav className="bg-white border-t border-gray-200 flex justify-around items-center py-3 pb-safe-area-inset-bottom">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <div className={`p-2 rounded-2xl transition-all ${isActive ? 'bg-sky-50 text-sky-500' : 'text-gray-300'}`}>
                <Icon size={22} className={isActive ? 'fill-sky-100' : ''} />
              </div>
              <span className={`text-[9px] font-black uppercase tracking-tighter ${isActive ? 'text-sky-600' : 'text-gray-400'}`}>
                {item.label}
              </span>
            </button>
          );
