...
const PhotoUploader: React.FC<PhotoUploaderProps> = ({ label, onImageSelect, aspectRatio = 'aspect-video' }) => {
  ...
  return (
    <div 
      ...
      className={`relative w-full ${aspectRatio} bg-gray-50 border-2 border-dashed rounded-[28px] overflow-hidden cursor-pointer transition-all group ${preview ? (qualityWarning ? 'border-amber-400 bg-amber-50' : 'border-sky-500 bg-sky-50') : 'border-gray-200 hover:border-sky-300 hover:bg-white'}`}
    >
      <input 
        type="file" 
        ref={fileInputRef} 
      ...
      {isAnalyzing ? (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-50/80 backdrop-blur-sm z-10">
           ...
           <span className="text-[10px] font-black text-sky-600 uppercase tracking-widest">Checking Quality...</span>
        </div>
      ) : preview ? (
        <>
